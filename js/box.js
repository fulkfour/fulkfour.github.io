...
// clone a static object (shallow copy)
...
var preloadEnd = function(id, state) {
    box.fire({type: state, component: 'preload', id: id});
    box.unbind('ready.preload.' + id, 'timeout.preload.' + id);
    // @todo see if necessary to kill each onload before deleting
    delete preloadCache[id];
};

var Preload = function(id, timeout) {
    this.id = id;
    this.timeout = timeout;
};
Preload.prototype = {
    start: function(url) {
        var that = this, i = url.length, l = i - 1, c = 0;
        ...
        this.timer = null;
        preloadEnd(this.id, state);
    }
};

box.preload = function(datas) {
    if(typeof datas.id == 'string') {
        preloadCache[datas.id] = new Preload(datas.id, datas.timeout || 20000);
        
        if(datas.img !== undefined && datas.img.constructor == Array && datas.img.length) {
            preloadCache[datas.id].start(datas.img);
        } else if(datas.element) {
            var url = [];
            $('img', datas.element).each(function(i, elm) {
                url[url.length] = elm.src;
            });
            
            if(url.length) {
                preloadCache[datas.id].start(url);
            } else {
                preloadEnd(datas.id, 'ready');
            }
        }
    }
};

// get URL params elements
...
$.fn.getAnId = function() {
    ...
    if(!id) {
        id = this.getBoxDatas('id') || '__n__' + (++uiCounter);
    }
    return id;
};
...
(function() {
    ...
    $(D).keydown(function(e) {
        if(e.which == 27 && components.length) {
        ...
        moveToElement: function(elm) {
            if(!this.disabled) {
                ...
                if(elm && elm.jquery && elm.length) {
					...
					var targetDim = elm.getHeight('margin-box');
...
(function() {
    ...
    var openTab = function(tabs) {
		...
		//get Statistics Tag(flag) from html, fire corresponding statistics code
		//statistics code are stored in "tabs.js"
		// statistics Tag(flag) in html (eg: <div id="tabContent2" class="tabContent" statTag="babyCurve">)
		var statFunction="tabs.stat."+box.dom("#"+tabs.active).attr("statTag")+"()";
		try{
			eval(statFunction);
		}
		catch(err){
    ...
    Tabs.prototype = {
        initialize: function(datas) {
            ...
            var directAccess = box.getURLHash('tab') || box.getURLParams().tab;
            if(directAccess) {
        ...
        open: function(id) {
            if(!this.waiting && id && id != this.active && D.getElementById(id)) {
                ...
                if(!this.animated) {
                    openTab(this);
					box.fire({type: 'open', component: 'tabs', id: this.id, source: this});
                }
            }
        },
        ...
        close: function() {
            if(this.active) {
                ...
                if(this.animated) {
                    ...
                    closeTab(this);
					box.fire({type: 'close', component: 'tabs', id: this.id, source: this});
                }
            }
        },
...
(function() {
    ...
    var openFauxOptions = function(select) {
        ...
        box.ui('generic.fauxOptions').setContent({
            html: '<ul>' + getFauxOptions(select) + '</ul>'
        }).addToDOM({
            target: box.ui('scroll.fauxOptions').wrapper
        });
        ...
        fauxOptions.width(fauxSelectPaddingBox.width-5);
        ...
        box.ui('generic.fauxOptionsMask').addToDOM({
            target: fauxOptions,
            method: 'before'
        }).applyStyles({
            width: wSize.width, height: 'document:content-box'
        });
    ...
    var closeFauxOptions = function(select) {
        ...
        box.ui('generic.fauxOptionsMask').removeFromDOM();
        if(W.ie6) {
            fauxOptions.find('li').unbind('mouseover').unbind('mouseout');
        }
        box.ui('scroll.fauxOptions').disable().reposition();
        box.ui('generic.fauxOptions').removeFromDOM();
        box.fire({type: 'close', component: 'field.select', id: select.name, source: select});
    };
    ...
    extend(SelectField, {
        ...
        addReplacement: function() {
            ...
            bindFieldFocusBlur(this);
            if(!fauxOptions && !box.ui('generic.fauxOptions') && !box.ui('generic.fauxOptionsMask')) {
...
(function() {
    ...
    uiCache.generic = {
        create: function(datas) {
            datas.id = datas.id || 'n' + (++counter);
            if(datas.id) {
                var id = 'generic.' + datas.id;
                return uiCache[id] || (uiCache[id] = new Generic(datas));
            }
        },
        ...
        destroy: function() {
            var i = arguments.length, id;
            while(i--) {
                id = 'generic.' + arguments[i];
                if(uiCache[id]) {
                    removeBindings(uiCache[id]);
                    delete uiCache[id];
                }
            }
        }
    ...
    var removeBindings = function(obj) {
        box.unbind(
            'requestsend.generic.' + obj.id,
            'requesterror.generic.' + obj.id,
            'requestsuccess.generic.' + obj.id,
            'contentready.generic.' + obj.id,
            'contentloaded.generic.' + obj.id,
            'timeout.generic.' + obj.id,
            'addtodom.generic.' + obj.id,
            'removefromdom.generic.' + obj.id,
            'ready.' + obj.id
        );
    };
    
    var getEventObject = function(obj, type) {
        return {
            type: type,
            component: 'generic',
            id: obj.id,
    ...
    var setLoaderReadyActions = function(obj) {
        var loader = {};
        loader['ready.preload.generic.' + obj.id + '.' + obj.ns] = function() {
            cache[obj.id][obj.ns].loaded = true;
            box.fire(getEventObject(obj, 'contentloaded'));
        };
        loader['timeout.preload.generic.' + obj.id + '.' + obj.ns] = function() {
            cache[obj.id][obj.ns].loaded = true;
            box.fire(getEventObject(obj, 'timeout'));
        };
        box.bind(loader);
    };
    
    var setContentReadyLoaded = function(obj, timeout) {
        cache[obj.id][obj.ns].ready = true;
        box.fire(getEventObject(obj, 'contentready'));
        box.preload({id: 'generic.' + obj.id + '.' + obj.ns, element: cache[obj.id][obj.ns].datas, timeout: timeout});
    };
    
    var Generic = function(datas) {
        this.id = datas.id;
        this.ns = 'default';
        cache[datas.id] = {};
        cache[datas.id].available = true;
        cache[datas.id]['default'] = {};
    };
    Generic.prototype = {
        add: function(name) {
            if(!this.has(name)) {
        ...
        remove: function(name) {
            if(this.has(name)) {
                delete cache[this.id][name];
            }
            return this;
        },
        
        use: function(name) {
            // cannot change object when processing another
            if(this.has(name) && this.isAvailable()) {
                this.ns = name;
            }
            return this;
        },
        ...
        has: function(name) {
            return (typeof cache[this.id][name] == 'object');
        },
        
        getContent: function() {
            return cache[this.id][this.ns].datas;
        },
        
        setContent: function(datas) {
            cache[this.id][this.ns].type = typeof datas.type == 'string' ? datas.type : 'html';
            var that = this, obj = cache[that.id][that.ns];
